...
def marshal_common_params(body):
    ...
    req_params["temperature"] = clamp(
        default(body, "temperature", req_params["temperature"]), 0.01, 1.99
    )  # fixup absolute 0.0/2.0
    ...
    req_params["repetition_penalty"] = default(
        body, "repetition_penalty", req_params["repetition_penalty"]
    )
    req_params["encoder_repetition_penalty"] = default(
        body, "encoder_repetition_penalty", req_params["encoder_repetition_penalty"]
    )
...
def get_technical_function():
    return {
        "name": "_message_to_user",
        "description": "This is a technical function to send a message to user. Use this only if there is no more appropriate function",
        "parameters": {
            "type": "object",
            "properties": {
                "message": {
...
def messages_to_prompt(body: dict, req_params: dict, max_tokens):
    ...
    if functions:
        ...
        if function_call != "none":
            if isinstance(function_call, dict):
                functions = list(
                    filter(lambda x: x["name"] == function_call["name"], functions)
                )
            else:
    ...
    if shared.settings["instruction_template"]:
        try:
            instruct = yaml.safe_load(
                open(
                    f"instruction-templates/{shared.settings['instruction_template']}.yaml",
                    "r",
                )
            )
            ...
            system_message_template = (
                instruct.get("system", "").replace("<|system-message|>", "{message}")
                or "{message}"
            )
            context_message_template = (
                instruct.get("context", "").replace("<|context-message|>", "{message}")
                or "{message}"
            )  # can be missing
            bot_start = template.find(
                "<|bot|>"
            )  # So far, 100% of instruction templates have this token
            ...
            bot_prompt = bot_message_template[
                : bot_message_template.find("{message}")
            ].rstrip(" ")
            functions_message_template = (
                instruct.get("functions", "") or "<|functions|>"
            )
            function_call_message_template = (
                instruct.get("function_call", "").replace("<|message|>", "{message}")
                or "{message}"
            )
            ...
            debug_msg(
                f"Loaded instruction role format: {shared.settings['instruction_template']}"
            )

        except Exception as e:
            req_params["stopping_strings"].extend(
                ["\nUser:", "User:"]
            )  # XXX User: prompt here also

            print(
                f"Exception: When loading instruction-templates/{shared.settings['instruction_template']}.yaml: {repr(e)}"
            )
    ...
    if "prompt" in body:
        context_msg = (
            end_line(role_formats["system"].format(message=body["prompt"]))
            + context_msg
        )
    ...
    if functions:
        ...
        system_msg += role_formats["functions"].replace("<|functions|>", functions_str)
...
def chat_completions(body: dict, is_legacy: bool = False) -> dict:
    ...
    req_params[
        "top_k"
    ] = 20  # There is no best_of/top_k param for chat, but it is much improved with a higher top_k.
    ...
    prompt, token_count = messages_to_prompt(
        body, req_params, max_tokens
    )  # updates req_params['stopping_strings']
    ...
    stop_reason = "stop"
    if (
        token_count + completion_token_count >= req_params["truncation_length"]
        or completion_token_count >= req_params["max_new_tokens"]
    ):
        stop_reason = "length"
    ...
    resp = {
        ...
        resp_list: [{"index": 0, "finish_reason": stop_reason, "message": message}],
        "usage": {
...
def stream_completions(body: dict, is_legacy: bool = False):
    ...
    if isinstance(prompt, list):
        if prompt and isinstance(prompt[0], int):
            ...
            raise InvalidRequestError(
                message="API Batched generation not yet supported.", param=prompt_str
            )
